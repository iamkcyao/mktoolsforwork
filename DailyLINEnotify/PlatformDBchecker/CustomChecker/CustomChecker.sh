source ~/.bash_profile
#!/bin/bash
dblist="./PlatformDBchecker/CustomChecker/customdb.list"
shell_folder="./PlatformDBchecker/CustomChecker"
...
then
	...
	then
		...
		echo " ${foldername} 尚未上傳至 S3 , 請稍後 , 檔案大小 ${TodayNasFileSize} " >> ${shell_folder}/notuploadok.log
	fi
fi
done

echo "${nowdate} 平台DB備份狀態" 
echo "-----------------------------------------------" 
nowupload=`ll ${shell_folder}/ |grep uploading|wc -l`
if [ ${nowupload} -eq 1 ]
then
echo "上傳中" 
cat ${shell_folder}/uploading.log 
echo "-----------------------------------------------" 
else
    continue
fi
...
then
    echo "待上傳" 
    cat ${shell_folder}/notuploadok.log 
else
        continue
fi
...
then
    echo "待上傳至NAS" 
    cat ${shell_folder}/nasnotfound.log 
    echo "-----------------------------------------------" 
else
        continue
fi
...
then
    ...
    #cat ${shell_folder}/uploadok.log 
    echo "-----------------------------------------------" 
    echo "全部檔案數 ${ListNum} 個 , 已完成 ${UploadOK} 個 , 待完成 ${NotUploadOK} 個 " 
