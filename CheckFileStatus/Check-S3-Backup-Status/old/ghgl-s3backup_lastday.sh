#備份相關資訊
iamaccount="ghglcloudwatch"
product_folder="ghgl-s3backup"
result_folder="./Check-S3-Backup-Status"
#定義今天日期格式
today=$(date +"%Y-%m-%d")
...
FileCount=$(aws s3 ls ${product_folder}/${FolderName["$i"]}/ --recursive --human-readable --summarize --profile=${iamaccount} | grep $yesterday | wc -l) ####計算今日新增的檔案數量

if [ ${FileCount} -gt 0 ]  ####若新增有1個檔案以上
then
    echo "${FolderName["$i"]} 今日新增 ${FileCount} 個檔案" >> ${result_folder}/${product_folder}/${product_folder}-${today}-lastday.log
fi

done
#COMMENT
####與前日數據比較####
TodayFile="${result_folder}/${product_folder}/${product_folder}-${today}-lastday.log"
...
then
	...
	then
		...
		echo "${getTItem} 相較昨日減少 ${ReduceNum} 個檔案。" >> ${result_folder}/${product_folder}/${product_folder}-compare-${today}-lastday.log
		
	elif [ ${FindYItemNum} -lt ${getTNum} ]  ####比較若昨日檔案數小於今日檔案數
	then
		IncreaseNum=`expr ${getTNum} - ${FindYItemNum}`
		echo "${getTItem} 相較昨日增加 ${IncreaseNum} 個檔案。" >> ${result_folder}/${product_folder}/${product_folder}-compare-${today}-lastday.log
	elif [ ${FindYItemNum} -eq ${getTNum} ]  ####比較若昨日檔案數等於今日檔案數
	then
		k=$(($k+1))
		if [ ${k} -eq ${TColCount} ]
		then
		echo "所有伺服器新增檔案數與前日相同。" >> ${result_folder}/${product_folder}/${product_folder}-compare-${today}-lastday.log
		fi
	fi
elif [ ${FindYItemCount} -eq 0 ]  ####若今日有伺服器名稱，前日無  則視為新增伺服器
then
	echo "本日新增 ${getTItem} , 新增 ${getTNum} 個檔案。" >> ${result_folder}/${product_folder}/${product_folder}-compare-${today}-lastday.log
...
then
	echo "本日移除 ${getYItem} 伺服器，可能已經關閉或故障，請與原廠或ＰＭ確認。" >> ${result_folder}/${product_folder}/${product_folder}-compare-${today}-lastday.log
